...
const modules = [
  { id: 'llm', icon: '✦', title: 'LLM fluency', time: '2–3 hrs', level: 'Foundation', blurb: 'Explain tokens, context, prompting, structured output, latency, and cost without hand-waving.', skills: ['Prompt anatomy', 'JSON outputs', 'Cost × latency'], quest: 'Turn a fuzzy customer ask into a tight system prompt and JSON schema.' },
  { id: 'rag', icon: '⌁', title: 'RAG systems', time: '4–6 hrs', level: 'Core', blurb: 'Design a grounded knowledge assistant and diagnose what broke when its answer is wrong.', skills: ['Embeddings', 'Chunking', 'Hybrid retrieval', 'Evaluation'], quest: 'Build the reference architecture and defend each retrieval decision.' },
  { id: 'agents', icon: '◈', title: 'Agents & workflows', time: '4–6 hrs', level: 'Core', blurb: 'Know when to automate deterministically and when a bounded agent adds real value.', skills: ['Tool calling', 'State', 'Human approval', 'Guardrails'], quest: 'Choose the right control flow for a support-triage scenario.' },
  { id: 'ship', icon: '⬡', title: 'Production AI', time: '3–4 hrs', level: 'Core', blurb: 'Make systems observable, evaluable, governed, secure, and safe to operate.', skills: ['Evals', 'Tracing', 'Prompt injection', 'Reliability'], quest: 'Write the go/no-go rubric for a customer-facing agent.' },
  { id: 'api', icon: '↔', title: 'APIs & automation', time: '4–6 hrs', level: 'Builder', blurb: 'Read JSON, call APIs, understand webhooks, and model automations in n8n.', skills: ['REST', 'Auth', 'Webhooks', 'Branches & retries'], quest: 'Map a lead-to-demo workflow from event to CRM update.' },
  { id: 'presales', icon: '◎', title: 'Solutions storytelling', time: '3–5 hrs', level: 'Primary track', blurb: 'Lead discovery, build compelling demos, and connect architecture choices to business outcomes.', skills: ['Discovery', 'Demo arc', 'POC scope', 'Partner enablement'], quest: 'Deliver a two-minute executive explanation of your design.' },
  { id: 'gtm', icon: '↗', title: 'GTM systems', time: '3–5 hrs', level: 'Optional track', blurb: 'Engineer the path from campaign signal to pipeline with measurement and automation.', skills: ['Routing', 'Enrichment', 'Funnel metrics', 'Experiments'], quest: 'Design an alert that turns falling conversion into an action.' },
]
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    {screen === 'map' && <LearningMap selected={selected} route={route} done={done} finish={finish} setScreen={setScreen} />}
...
function LearningMap({ selected, route, done, finish, setScreen }) {
  const [open, setOpen] = useState(route[0]?.id)
  const active = modules.find(m => m.id === open)
  return <main className="page"><section className="page-intro"><p className="eyebrow">{selected.title.toUpperCase()}</p><h1>The learning map</h1><p>Take the missions in order or jump ahead. Finish means: you can explain the idea, make a small thing, and talk through its trade-offs.</p></section><section className="map-layout"><div className="mission-list">{route.map((m, i) => <button key={m.id} className={`mission ${open === m.id ? 'open' : ''} ${done[m.id] ? 'done' : ''}`} onClick={() => setOpen(m.id)}><span className="mission-number">{done[m.id] ? '✓' : String(i + 1).padStart(2, '0')}</span><span className="mission-icon">{m.icon}</span><span><b>{m.title}</b><small>{m.time} · {m.level}</small></span><span>›</span></button>)}</div><article className="mission-detail" key={active.id}><div className="detail-top"><span className="big-icon">{active.icon}</span><div><p className="eyebrow">MISSION {route.findIndex(m => m.id === active.id) + 1}</p><h2>{active.title}</h2><p>{active.blurb}</p></div></div><div className="skill-pills">{active.skills.map(s => <span key={s}>{s}</span>)}</div><div className="quest-box"><span>QUEST BRIEF</span><p>{active.quest}</p></div><div className="detail-actions"><button className={done[active.id] ? 'completed' : 'primary'} onClick={() => finish(active.id)}>{done[active.id] ? 'Completed — undo' : 'Mark quest complete +120 XP'}</button><button className="ghost" onClick={() => setScreen('resources')}>Find free course</button></div><p className="coach-note"><b>Coach note:</b> aim for “clear and defensible,” not “I memorised every framework.” Strong AI-role candidates identify the business goal, set sensible constraints, and communicate what they would test next.</p></article></section></main>
}
